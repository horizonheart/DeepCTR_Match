    ...
    def call(self, seq_value_len_list, mask=None, **kwargs):
        if self.supports_masking:
            ...
            mask = tf.sequence_mask(user_behavior_length,
                                    self.seq_len_max, dtype=tf.float32)#todo (？,1,16)
            mask = tf.transpose(mask, (0, 2, 1))#todo (？,16,1)

        embedding_size = uiseq_embed_list.shape[-1]#todo uiseq_embed_list (?,16,8)

        mask = tf.tile(mask, [1, 1, embedding_size])

        uiseq_embed_list *= mask
        hist = uiseq_embed_list #todo uiseq_embed_list(?,16,8)
        if self.mode == "max":
...
#todo 序列特征 每个特征的embedding乘以对应的权值
class WeightedSequenceLayer(Layer):
    ...
    # 这是你定义权重的地方。这个方法必须设self.built = True，可以通过调用super([Layer], self).build()
    # 完成。build，这个函数用来确立这个层都有哪些参数，哪些参数是可训练的哪些参数是不可训练的。
    #todo 1.0 初始化完成后就调用这个函数
    # input_shape[0] (?,16,8)
    # input_shape[1] (?,1)
    # input_shape[2] (?,16,1)
    #input_shape 就是[embedding_dict[feature_name], features[feature_length_name], features[fc.weight_name] 模型传入的值
    def build(self, input_shape):
        if not self.supports_masking:
            self.seq_len_max = int(input_shape[0][1])
    ...
    def call(self, input_list, mask=None, **kwargs):
        if self.supports_masking:
            ...
            mask = tf.sequence_mask(key_length_input,
                                    self.seq_len_max, dtype=tf.bool) #todo 根据输入 前几个位置的元素为true 之后的为false (?,1,16)
...
class AttentionSequencePoolingLayer(Layer):
    ...
    def call(self, inputs, mask=None, training=None, **kwargs):
        ...
        if self.supports_masking:
            if mask is None:
                raise ValueError(
                    "When supports_masking=True,input must support masking")
            queries, keys = inputs
            key_masks = tf.expand_dims(mask[-1], axis=1) #todo 计算真实的mask信息
        ...
        # todo 调用din 网络 attention_score=（?,4,1）
        attention_score = self.local_att([queries, keys], training=training)

        outputs = tf.transpose(attention_score, (0, 2, 1)) #(?,1,4)
        ...
        if not self.return_score:
            outputs = tf.matmul(outputs, keys)#todo 根据学习到的权重，对keys进行加权
